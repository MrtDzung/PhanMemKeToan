      ...
      <div class="table-container flex-1 bg-[var(--surface-card)] border-round border-1 border-solid border-[var(--surface-border)] overflow-hidden">
        <p-table [value]="items()" [loading]="loading()" dataKey="id" editMode="row" styleClass="p-datatable-sm p-datatable-gridlines p-datatable-striped" [scrollable]="true" scrollHeight="flex">
          <ng-template pTemplate="header">
            <tr>
              <th style="width:150px; background: var(--surface-ground); font-weight: 600; text-align: left;">Mã kho *</th>
              <th style="width:250px; background: var(--surface-ground); font-weight: 600; text-align: left;">Tên kho *</th>
              <th style="background: var(--surface-ground); font-weight: 600; text-align: left;">Địa chỉ</th>
              ...
              <td>
                <p-cellEditor>
                  <ng-template pTemplate="input">
                    <input pInputText type="text" [(ngModel)]="row.warehouseCode" class="w-full p-inputtext-sm" style="font-size: 12px;" maxlength="20" />
                  </ng-template>
                  <ng-template pTemplate="output">{{row.warehouseCode}}</ng-template>
                </p-cellEditor>
              </td>
              <td>
                <p-cellEditor>
                  <ng-template pTemplate="input">
                    <input pInputText type="text" [(ngModel)]="row.warehouseName" class="w-full p-inputtext-sm" style="font-size: 12px;" />
                  </ng-template>
                  <ng-template pTemplate="output">{{row.warehouseName}}</ng-template>
                </p-cellEditor>
              </td>
              <td>
                <p-cellEditor>
                  <ng-template pTemplate="input">
                    <input pInputText type="text" [(ngModel)]="row.address" class="w-full p-inputtext-sm" style="font-size: 12px;" />
                  </ng-template>
                  <ng-template pTemplate="output">{{row.address}}</ng-template>
                </p-cellEditor>
              </td>
...
export class WarehousesPageComponent implements OnInit {
  ...
  readonly items = signal<WarehouseDto[]>([]);
  ...
  load() {
    ...
    this.api.getWarehouses().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => {
        this.items.set(data);
      ...
      error: () => {
        this.loading.set(false);
        this.msgService.add({ severity: 'error', summary: 'Lỗi', detail: 'Không thể tải danh sách kho' });
      }
    });
  }
  ...
  addRow() {
    ...
    this.items.set([
      { id: '', warehouseCode: '', warehouseName: '', address: '', warehouseKeeper: '', isActive: true },
      ...list
    ]);
  }

  cancelEdit(row: WarehouseDto, index: number) {
    if (!row.id) {
      const list = [...this.items()];
      list.splice(index, 1);
      this.items.set(list);
  ...
  saveRow(row: WarehouseDto) {
    if (!row.warehouseCode?.trim() || !row.warehouseName?.trim()) {
    ...
    this.api.upsertWarehouse(payload).subscribe({
      next: () => {
        this.msgService.add({ severity: 'success', summary: 'Thành công', detail: 'Đã lưu kho' });
        this.load();
      },
      error: () => {
