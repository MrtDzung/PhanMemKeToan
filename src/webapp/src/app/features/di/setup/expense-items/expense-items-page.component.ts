...
import { ExpenseItemDto } from '../../models/master-data.models';

@Component({
  selector: 'app-expense-items-page',
  standalone: true,
      ...
      <div class="table-container flex-1 bg-[var(--surface-card)] border-round border-1 border-solid border-[var(--surface-border)] overflow-hidden">
        <p-table [value]="items()" [loading]="loading()" dataKey="id" editMode="row" styleClass="p-datatable-sm p-datatable-gridlines p-datatable-striped" [scrollable]="true" scrollHeight="flex">
          <ng-template pTemplate="header">
            <tr>
              <th style="width:150px; background: var(--surface-ground); font-weight: 600; text-align: left;">Mã khoản mục *</th>
              <th style="background: var(--surface-ground); font-weight: 600; text-align: left;">Tên khoản mục *</th>
              ...
              <td>
                <p-cellEditor>
                  <ng-template pTemplate="input">
                    <input pInputText type="text" [(ngModel)]="row.expenseCode" class="w-full p-inputtext-sm" style="font-size: 12px;" maxlength="20" />
                  </ng-template>
                  <ng-template pTemplate="output">{{row.expenseCode}}</ng-template>
                </p-cellEditor>
              </td>
              <td>
                <p-cellEditor>
                  <ng-template pTemplate="input">
                    <input pInputText type="text" [(ngModel)]="row.expenseName" class="w-full p-inputtext-sm" style="font-size: 12px;" />
                  </ng-template>
                  <ng-template pTemplate="output">{{row.expenseName}}</ng-template>
                </p-cellEditor>
              </td>
...
export class ExpenseItemsPageComponent implements OnInit {
  private readonly api = inject(LookupsApiService);
  private readonly confirmService = inject(ConfirmationService);
  private readonly msgService = inject(MessageService);
  private readonly destroyRef = inject(DestroyRef);

  readonly items = signal<ExpenseItemDto[]>([]);
  ...
  load() {
    ...
    this.api.getExpenseItems().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data) => {
        this.items.set(data);
      ...
      error: () => {
        this.loading.set(false);
        this.msgService.add({ severity: 'error', summary: 'Lỗi', detail: 'Không thể tải danh sách khoản mục' });
      }
    });
  }
  ...
  addRow() {
    ...
    this.items.set([
      { id: '', expenseCode: '', expenseName: '', isActive: true },
      ...list
    ]);
  }

  cancelEdit(row: ExpenseItemDto, index: number) {
    if (!row.id) {
      const list = [...this.items()];
      list.splice(index, 1);
      this.items.set(list);
  ...
  saveRow(row: ExpenseItemDto) {
    if (!row.expenseCode?.trim() || !row.expenseName?.trim()) {
    ...
    this.api.upsertExpenseItem(payload).subscribe({
      next: () => {
        this.msgService.add({ severity: 'success', summary: 'Thành công', detail: 'Đã lưu khoản mục' });
        this.load();
      },
      error: () => {
  ...
  deleteRow(row: ExpenseItemDto) {
    if (!row.id) return;
    this.confirmService.confirm({
      message: 'Bạn có chắc muốn xóa?',
      accept: () => {
        this.api.deleteExpenseItem(row.id).subscribe({
          next: () => {
